 ...
 * @license MIT
 *
 * There are a number of things that must be handled by an external parser:
 * - the COMMENT multi-line directive
 * - distinguishing instruction mnemonics from regular identifiers (there's way too many
 *   for just this tree-sitter grammar to handle; over 1,400)
 */

/// <reference types="tree-sitter-cli/dsl" />
// @ts-check

// the following mnemonics were taken from tree-sitter-x86asm (https://github.com/bearcove/tree-sitter-x86asm/blob/9d02829/grammar.js) which is under the Apache 2.0 and MIT licenses
  ...
  rules: {
    ...
    _asm_instruction: ($) =>
      seq(
        ...
        optional(field("args", optional($.expr_list))),
      ),
    instruction: ($) =>
    ...
    in_seg_dir_list: ($) => prec.left(repeat1($._in_seg_dir)),
